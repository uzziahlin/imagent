...
- [ ] **P1-H  媒体解密 silent failure**：`crates/ilink/src/media.rs:237-245`。`aes_key` parse 失败时密文当明文返回。修复：fail-closed 返回 Err。
- [ ] **P1-I  WeCom 完全无消息去重**：`crates/wecom/src/proto.rs:53`。复用 ilink 的 `Dedup`。
- [ ] **P1-J  `login.rs` reqwest 未禁 redirect**：`crates/ilink/src/login.rs:38-43`。与运行时 client 不一致。
- [ ] **P1-K  `/compact` 首条消息失败 → compact_summary 永久丢失**：`crates/core/src/dispatch.rs:773-783`。摘要删除推迟到 run 成功后。
- [ ] **P1-L  媒体下载 chunked OOM**：`crates/ilink/src/media.rs:222-235`。改流式 `bytes_stream()` + `take(max)`。
