...
/// v10：sessions 增加 task_todos 列（TaskList 预热——每会话最近一份全量任务
/// 快照 JSON，含真实任务 id；轮首播种给 backend 待办累积器）。存量行 NULL
/// （冷启动首轮走转录兜底或空降级）。
pub const SCHEMA_V10: &str = "ALTER TABLE sessions ADD COLUMN task_todos TEXT;";

/// v11：cron 定时任务表（v1.18 /cron）——重启后调度器重载并按表达式重算
/// next_run；停机期间错过的不补跑（到期首个 tick 触发一次后顺延）。
pub const SCHEMA_V11: &str = "CREATE TABLE IF NOT EXISTS cron_jobs (
  id         TEXT PRIMARY KEY,
  conv       TEXT NOT NULL,
...
pub fn migrate(conn: &rusqlite::Connection) -> rusqlite::Result<()> {
    ...
    if current < 10 {
        ...
        tx.pragma_update(None, "user_version", 10_i64)?;
    }
    if current < 11 {
