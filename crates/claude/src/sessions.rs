...
/// session_id，imagent 按 P5-5 落库后成为「幽灵会话」——下次 `--resume` 得到
...
pub(crate) fn session_exists(workdir: &Path, session_id: &str) -> bool {
    // 真机校准补丁：default_claude_dir 返回 ~/.claude，会话文件在 projects/ 子层
    // ——少拼这一段会把所有会话误判为幽灵（每轮开新会话、上下文全丢）。
    default_claude_dir()
